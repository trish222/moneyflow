...
async function seed() {
    try {
        ...
        await pool.query(`INSERT INTO "User" (id, email, "passwordHash", name, "createdAt", "updatedAt") VALUES ($1, $2, $3, $4, NOW(), NOW()) ON CONFLICT (id) DO NOTHING`, [DEFAULT_USER_ID, "trish@example.com", "$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcg7b3XeKeUxWdeS86E36CHqnAs", "Trish Nguyen"]);
       ...
       VALUES ($1, $2, $3, $4, NOW(), NOW()) ON CONFLICT DO NOTHING`, [DEFAULT_USER_ID, "Savings", 15000, "savings"]);
        ...
        for (let i = 0; i < transactions.length; i++) {
         ...
         VALUES ($1, $2, $3, $4, $5, NOW()) ON CONFLICT DO NOTHING`, [DEFAULT_USER_ID, tx.amount, tx.type, tx.category, date]);
        }
        ...
        for (const account of investmentAccounts) {
            const result = await pool.query(`INSERT INTO "InvestmentAccount" ("userId", name, "accountType", "createdAt", "updatedAt")
         VALUES ($1, $2, $3, NOW(), NOW()) ON CONFLICT DO NOTHING
         RETURNING id`, [DEFAULT_USER_ID, account.name, account.accountType]);
        ...
        if (accountIds.length === 0) {
            const existingAccounts = await pool.query(`SELECT id FROM "InvestmentAccount" WHERE "userId" = $1 ORDER BY "createdAt" ASC`, [DEFAULT_USER_ID]);
            accountIds.push(...existingAccounts.rows.map((row) => row.id));
        }
        ...
        for (const inv of investments) {
            ...
            await pool.query(`INSERT INTO "Investment" ("userId", "investmentAccountId", name, value, type, date, "createdAt", "updatedAt")
         VALUES ($1, $2, $3, $4, $5, NOW(), NOW(), NOW()) ON CONFLICT DO NOTHING`, [DEFAULT_USER_ID, accountId, inv.name, inv.value, inv.type]);
        }
