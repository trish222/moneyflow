...
function isHeaderRow(row: string[]): boolean {
  ...
  const matches = lowerRow.filter((cell) =>
...
/**
 * Find column indexes for known headers
 */
...
function detectHeaderIndexes(headers: string[]): HeaderIndexes {
  const indexes: HeaderIndexes = {};

  headers.forEach((header, idx) => {
    const h = header.toLowerCase();
    if (h.includes("date") || h.includes("time")) indexes.dateIdx = idx;
    else if (h.includes("amount") || h.includes("total")) indexes.amountIdx = idx;
    else if (h.includes("type") || h.includes("transaction")) indexes.typeIdx = idx;
    else if (h.includes("category") || h.includes("class")) indexes.categoryIdx = idx;
    else if (
...
function parseRow(
  ...
  if (headerIndexes) {
    ...
    if (headerIndexes.dateIdx !== undefined && headerIndexes.amountIdx !== undefined) {
      ...
      if (amountCell) amount = parseFloat(amountCell);
    } else {
    ...
    if (headerIndexes.descriptionIdx !== undefined) {
      ...
      if (descCell) description = descCell;
    }
  } else {
  ...
  if (!isValidDate(date)) throw new Error(`Invalid date format: ${date}`);
  if (isNaN(amount)) throw new Error(`Invalid amount: ${cells[1]}`);
